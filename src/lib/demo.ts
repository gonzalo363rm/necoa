...
// Historial demo (meses anteriores) para tendencias
...
for (let i = 1; i <= 5; i++) {
  ...
  DEMO_TRANSACTIONS.push(
    ...
    },
    {
      ...
      installment_total: null,
      category_override: null,
      splits: [{ user_id: partnerId, share_pct: 100 }],
    },
    {
      id: `tx-hist-food-${i}`,
      family_id: DEMO_FAMILY.id,
      type: 'expense',
      amount: Math.round(150000 * factor),
      currency: 'ARS',
      occurred_at: `${ym}-06`,
      paid_by: demoUser.id,
      created_by: demoUser.id,
      ...
      splits: [
        { user_id: demoUser.id, share_pct: 50 },
        { user_id: partnerId, share_pct: 50 },
      ],
    },
    {
      id: `tx-hist-rest-${i}`,
      family_id: DEMO_FAMILY.id,
      type: 'expense',
      amount: Math.round(40000 * (1.15 - i * 0.05)),
      currency: 'ARS',
      occurred_at: `${ym}-12`,
      paid_by: partnerId,
      created_by: partnerId,
